...
# ╔═╡ 98042020-824b-11ef-1ef2-13a24d286bb4
begin
	using Clarabel
	using SumOfSquares
...
# ╔═╡ 30068e32-b4b1-418c-b700-14d1df3e94fb
begin
	R"library(momentLS)"
	R"set.seed(12345)"
	R"chainParams = list(type = 'AR', rho = 0.5, M = 10000)"
	R"ch = generateChain(chainParams)"
	R"x = ch$x"
	R"r = autocov(x)"
	R"delta = tune_delta(x, nSplits = 5, c_M_const = 0)$delta*0.8"
	@rget r
	@rget delta
	
end

# ╔═╡ b8e00cdd-9803-4e8b-bada-21cd33b1de6c
transform = function(x,i)
	return(sign(i)*((1+x)*(1-2.0^-abs(i)) - x))
end

# ╔═╡ 742fbd00-2c27-4f50-9ca5-23657344d9a1
grad_optimize = function(r,p, supp, weight, solver)
	gradients = zeros(26)
	supports = zeros(26)
	
	for ind in zip([1:1:18;],append!([1:1:9;], [-9:1:-1;]))
...
# ╔═╡ 491a198a-0631-4905-a3b4-baee485ccad6
...
# ╔═╡ 9b3539a4-c6b1-4323-b550-2599f1a5ee87
...
# ╔═╡ 2fded22a-eda3-4f63-943c-711d0e129d2f
p2 = estimate_poly(2,r)

# ╔═╡ 26fb7dca-986c-4c33-af90-42ffae5ed664
# ╠═╡ disabled = true
#=╠═╡
begin
	exponents = [0:1:p2[3]-1;]
	sum(p2[1] .* (0.69-0.75).^exponents)
end
  ╠═╡ =#

# ╔═╡ 48867912-e77e-43b0-8e34-1429522eb165
estim_poly = function(s,x)
	exponents = [0:1:s[3]-1;]
	return sum(s[1] .* (x-s[2]).^exponents)
end

# ╔═╡ cf8169bd-3432-4c25-bfd8-a9445f98812c
# ╠═╡ disabled = true
#=╠═╡
points = [0.5:0.01:0.75;]
  ╠═╡ =#

# ╔═╡ d8a3c978-381d-4e0a-863b-977ca08d54c2
#=╠═╡
graph = [estim_poly(p2,pt) for pt in points]
  ╠═╡ =#

# ╔═╡ 73eb3e1c-13c5-4e61-9be9-fa4640f67c03
#=╠═╡
plot(points,graph)
  ╠═╡ =#

# ╔═╡ 5982540c-776f-4da8-bb3e-4f9cac486253
b = zeros(p2[3])

# ╔═╡ 8839329f-573b-4e83-909b-f6ef27abd62f
for j in 0:length(b) -1
	b[j+1] = (-1)^j * big((1-0.75)^j) * big(p2[1][j+1])
end

# ╔═╡ 71dc2050-3d54-465f-b0de-5f59702f62ce
p3 = estimate_poly(3,r)

# ╔═╡ 0ce9a27e-b82d-498c-b651-2355720e732a
exponents = [0:1:p3[3]-1;]

# ╔═╡ b97fde0f-ee48-4673-8099-6ef516a32d3e
@polyvar x

# ╔═╡ d22c6df8-a7ae-4dcb-ba99-32642e123dd4
p = sum(p3[1] .* x.^exponents)

# ╔═╡ 92bb4ba3-0df7-4edb-9211-b5996b7bc31b
p(0.8 - p3[2])

# ╔═╡ 6b14f874-a010-43cb-bbc9-052c7cf5890c
# ╠═╡ disabled = true
#=╠═╡
l = []
  ╠═╡ =#

# ╔═╡ 8e09db20-1487-4761-8df9-adea2b9c98c3
final = estimate_poly(13,r)

# ╔═╡ 3fc96e2f-e5a1-4fb5-b08b-e7f2f719b8be
final[2]

# ╔═╡ cd7a3a43-b0a9-4f1b-b1dd-ea51341aadd8
1-2.0^(-12)

# ╔═╡ 82f8c653-7160-42f2-abe4-e2e0f98d5e6b
point = 0.99979

# ╔═╡ b1c41b5d-6712-4bce-a534-e1d01104979d
sum(final[1] .* ((final[2] - point)/(1-final[2])) .^[0:1:final[3]-1;])

# ╔═╡ 2c68e49f-5a37-4518-bfa2-b6324b9b9edc
final[1]

# ╔═╡ d5b491a9-1cca-40b0-a916-2fd7d915fcb6
point-final[2]

# ╔═╡ 3fbada9f-74f9-424f-9ad2-6e7d8d73e48e
findmax(final[1])

# ╔═╡ 2619ee01-4388-4c54-a5da-d5eef7553851
sum(r .* point .^[0:1:9999;])

# ╔═╡ 80a1db38-2ea5-4f78-bf5e-b374818c344a
pol = sum(final[1] .* x .^ [0:1:final[3]-1;])

# ╔═╡ e8398ed8-901e-4c2a-b6b0-3475fdd52ff5
pol(0.6)

# ╔═╡ 591f950f-b15c-4075-bf09-86d3132b8c75
second = 1.6*final[2] - 0.6

# ╔═╡ 44aacc3e-4c27-4837-99c4-91858cd5389a
sum(r .* second .^[0:1:9999;])

# ╔═╡ 798091db-eb17-49c8-9fb9-aaec92c852bd
id = append!([1:1:13;], [-13:1:-1;])

# ╔═╡ 908bf635-a788-4955-9000-2daf0390740c
dictionary = Dict(id .=> [estimate_poly(i,r) for i = append!([1:1:13;],[-13:1:-1;])])

# ╔═╡ 7b34dd0a-1ae6-4386-b82d-118badf3d01e
dictionary[1][2]

# ╔═╡ e712c321-6958-45cf-936c-690e0189c8de
estpoly = estimate_poly(-1,r)

# ╔═╡ 86c77e05-3a7e-47a5-8ad3-f517a02014ce
sum(estpoly[1] .* 0.4 .^[0:1:estpoly[3]-1;])

# ╔═╡ 16979aa6-39cb-479e-91aa-2808ae733929
estpoly[2]

# ╔═╡ 87ab2873-f061-4e86-b1e6-74b59e98b206
sum(r .* (-0.3).^[0:1:9999;])

# ╔═╡ f3ebf2cc-2178-462a-9c89-d499a0544adf
l1 = [1:1:13;]; l2 = [-13:1:-1;]

# ╔═╡ 00ad2068-11be-4732-93c2-ba7e02ba53d2
for i in zip([1:1:26;], append!(l1,l2))
	print(r[i[1]])
end

# ╔═╡ 6bae9030-daf4-4db0-b966-af7dfb119359
solver = Clarabel.Optimizer

# ╔═╡ 05b92c08-5764-4cd4-a78f-b64a6029e9e0
begin
	supp = [rand(Uniform(-1,1))]
		ex = [0:1:9999;]
		ai = 2*sum((supp[1].^ex).*r) - r[1]
		Bi = (1 + supp[1]^2)/(1-supp[1]^2)
	
		weight = [ai/Bi]
end

# ╔═╡ 60279c20-1bf5-4445-8602-5184adf74371
supp

# ╔═╡ 4632de32-859d-4b9c-963f-3ff70f6b7ddd


# ╔═╡ 5177eeeb-345f-4089-9be0-eba081208193
supp

# ╔═╡ 3c5617c2-32c0-4cdb-ae6b-d5e0a2139126
weight

# ╔═╡ 770d00c8-e037-4621-91bd-e9a919c8568a
momentLSmod = function(r, tol,dictionary)
	pts = []
	solver = Clarabel.Optimizer
	n = length(r)
	ln = Int(ceil(log(n)))
	...
	while(count < 10 && !conv)
		...
		points = grad_optimize(r, dictionary, supp, weight, solver)
...
manifest_format = "2.0"
project_hash = "82a15618412f04737e4e07753c43f0447c947ed0"
...
deps = ["LinearAlgebra", "MultivariatePolynomials", "MutableArithmetics"]
git-tree-sha1 = "0974af2af8122b6844af29cd109acf5103c90cb7"
uuid = "be282fd4-ad43-11e9-1d11-8bd9d7e43378"
version = "0.2.2"

[[deps.MultivariateMoments]]
deps = ["Colors", "CommonSolve", "LinearAlgebra", "MultivariateBases", "MultivariatePolynomials", "MutableArithmetics", "REPL", "RecipesBase", "RowEchelon", "SemialgebraicSets", "SparseArrays"]
git-tree-sha1 = "1c95138deba586047ed20a1efb72ee01296dc208"
...
deps = ["GroupsCore", "PrecompileTools", "Random"]
git-tree-sha1 = "68cfeef91301414529f8dfeac815663989bcb832"
...
[[deps.PolyJuMP]]
deps = ["DataStructures", "DynamicPolynomials", "IntervalArithmetic", "JuMP", "LinearAlgebra", "MathOptInterface", "MultivariateBases", "MultivariateMoments", "MultivariatePolynomials", "MutableArithmetics", "SemialgebraicSets"]
git-tree-sha1 = "0cc46e4f7ac3e6b745e9d033dbef362382ffe082"
...
git-tree-sha1 = "b8630408a627dc5f9a8019bf5b4bab8ef5ad1f5d"
...
[[deps.SumOfSquares]]
deps = ["DataStructures", "JuMP", "LinearAlgebra", "MathOptInterface", "MultivariateBases", "MultivariateMoments", "MultivariatePolynomials", "MutableArithmetics", "PolyJuMP", "Reexport", "SemialgebraicSets", "SparseArrays", "SymbolicWedderburn"]
git-tree-sha1 = "cc2360097de794086a4c6de873de2dfd7cdbfb49"
...
deps = ["Cyclotomics", "GroupsCore", "LinearAlgebra", "PermutationGroups", "PrecompileTools", "Primes", "SparseArrays", "StarAlgebras"]
git-tree-sha1 = "bad2f27c62f19bfe7fa0901fba6c6e004f74adfc"
